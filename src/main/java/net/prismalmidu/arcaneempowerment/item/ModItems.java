package net.prismalmidu.arcaneempowerment.item;

import net.minecraft.world.item.BucketItem;
import net.minecraft.world.item.Item;
import net.minecraft.world.item.Items;
import net.minecraftforge.eventbus.api.IEventBus;
import net.minecraftforge.registries.DeferredRegister;
import net.minecraftforge.registries.ForgeRegistries;
import net.minecraftforge.registries.RegistryObject;
import net.prismalmidu.arcaneempowerment.ArcaneEmpowerment;
import net.prismalmidu.arcaneempowerment.fluid.ModFluids;
import net.prismalmidu.arcaneempowerment.item.custom.PerkEssenceFoodItem;

public class ModItems {
    public static final DeferredRegister<Item> ITEMS =
            DeferredRegister.create(ForgeRegistries.ITEMS, ArcaneEmpowerment.MOD_ID);

    public static final RegistryObject<Item> ARCANIUM_SHARD = ITEMS.register("arcanium_shard",
            () -> new Item(new Item.Properties()));
    public static final RegistryObject<Item> PHANTASMIUM_SHARD = ITEMS.register("phantasmium_shard",
            () -> new Item(new Item.Properties()));
    public static final RegistryObject<Item> STELLARIUM_SHARD = ITEMS.register("stellarium_shard",
            () -> new Item(new Item.Properties()));
    public static final RegistryObject<Item> CHRONIUM_SHARD = ITEMS.register("chronium_shard",
            () -> new Item(new Item.Properties()));
    public static final RegistryObject<Item> VOLSARNIUM_SHARD = ITEMS.register("volsarnium_shard",
            () -> new Item(new Item.Properties()));
    public static final RegistryObject<Item> LIQUID_MANA_BUCKET = ITEMS.register("liquid_mana_bucket",
            () -> new BucketItem(ModFluids.SOURCE_LIQUID_MANA, new Item.Properties().craftRemainder(Items.BUCKET).stacksTo(1)));

    public static final RegistryObject<Item> PERK_FOOD_T2 = ITEMS.register("perk_food_tier2",
            () -> new PerkEssenceFoodItem(new Item.Properties(), 1, 1));
    public static final RegistryObject<Item> PERK_FOOD_T3 = ITEMS.register("perk_food_tier3",
            () -> new PerkEssenceFoodItem(new Item.Properties(), 2, 1));
    public static final RegistryObject<Item> PERK_FOOD_T4 = ITEMS.register("perk_food_tier4",
            () -> new PerkEssenceFoodItem(new Item.Properties(), 3, 1));
    public static final RegistryObject<Item> PERK_FOOD_T5 = ITEMS.register("perk_food_tier5",
            () -> new PerkEssenceFoodItem(new Item.Properties(), 4, 1));

    public static void register(IEventBus eventBus) {
        ITEMS.register(eventBus);
    }
}
